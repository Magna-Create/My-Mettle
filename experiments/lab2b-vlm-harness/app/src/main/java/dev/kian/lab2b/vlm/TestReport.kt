...
object TestReport {
    ...
    fun create(request: HarnessSnapshot, result: HarnessSnapshot, turn: InferenceTurn?, image: SelectedImageInfo?, evidence: OcrEvidence?,
        ...
        return JSONObject().put("schema", 1).put("test_id", java.util.UUID.randomUUID().toString())
            ...
            .put("crop_preparation_ms", if (stage == "LOCALISE") 0 else request.cropPreparationMs)
            .put("crop_origin", if (stage == "LOCALISE") "FULL_FRAME" else request.cropOrigin)
            .put("crop", (if (stage == "LOCALISE") null else request.crop)?.let { JSONObject().put("label", it.label).put("ratios", JSONArray(listOf(it.left,it.top,it.right,it.bottom))) } ?: JSONObject.NULL)
            .put("localisation_report", if (stage == "LOCALISE" || request.cropOrigin != "MODEL_PROPOSED_HUMAN_REVIEWED" || request.localisationReport.isEmpty()) JSONObject.NULL else JSONObject(request.localisationReport))
            ...
            .put("first_raw_output_ms", result.timing.firstRawOutputMs ?: JSONObject.NULL)
