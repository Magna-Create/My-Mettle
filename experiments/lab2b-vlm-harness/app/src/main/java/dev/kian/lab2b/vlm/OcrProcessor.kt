...
object OcrProcessor {
    suspend fun recognise(image: SelectedImageInfo): OcrEvidence {
        ...
        try {
            ...
            return OcrEvidence(text.text, text.textBlocks.map { block ->
                OcrBlock(block.text, block.boundingBox.box(), block.cornerPoints.points(), block.recognizedLanguage,
                    block.lines.map { line -> OcrLine(line.text, line.boundingBox.box(), line.cornerPoints.points(), line.recognizedLanguage) })
