...
enum class WeightOrigin { RECOGNISED_KG, CHARACTER_CORRECTION, UNIT_CANDIDATE, HUMAN_EDITED }
...
object WeightOcrParser {
    ...
    fun parse(evidence: OcrEvidence, part: CapturePart): WeightParse {
        ...
        lines.forEachIndexed { index, line ->
            val compact = line.text.trim().replace(Regex("\\s+"), " ")
            ...
            val origin = when { !unitExact -> WeightOrigin.UNIT_CANDIDATE; changes.isNotEmpty() -> WeightOrigin.CHARACTER_CORRECTION; else -> WeightOrigin.RECOGNISED_KG }
