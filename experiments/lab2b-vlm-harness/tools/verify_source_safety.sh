...
if grep -R --line-number -E 'dev\.kian\.mymettle|project\(["'"']?:app|Room|DataStore|CameraX|mlkit|retrofit|okhttp|hilt|koin' \
    "$HARNESS/app/src" "$HARNESS/app/build.gradle"; then
    fail "forbidden My Mettle/product/runtime dependency marker found in harness source"
fi
