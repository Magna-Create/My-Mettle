...
0.6 adds [number-column selection and explicit units](research/LAB_2B_OCR_COLUMNS.md), including a sloping manual strip, nearby small unit labels and row-based filter comparison. Unitless values are preserved; kg/lb decisions have provenance. First build 34058610064 passed 73 tests, lint, APK and native/Termux checks. Additional damaged-unit regression coverage is undergoing final validation. Physical acceptance pending; no machine masking or LAB-2C work.
